  ...
  if (collapsed) return (
    <div style={{ borderTop: '1px solid #252d47', paddingTop: 8, paddingBottom: 10, width: '100%', display: 'flex', justifyContent: 'center' }}>
      <button
        ...
        style={{
          ...iconBtn,
          color: isOpen ? '#60a5fa' : '#7c8db5',
          background: isOpen ? 'rgba(96,165,250,.12)' : 'none',
          borderRadius: 6,
        }}
      >
        <BookIcon />
    ...
    <div style={{ borderTop: '1px solid #252d47', padding: '8px 10px', flexShrink: 0 }}>
      ...
      >
        <div style={{
          ...
          background: isOpen ? '#1d4ed8' : '#2a3558',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          <BookIcon />
        </div>
        ...
        {isOpen && (
          <span style={{ marginLeft: 'auto', fontSize: 10, background: '#1d4ed8', color: '#fff', padding: '1px 6px', borderRadius: 99, fontWeight: 700 }}>ON</span>
        )}
      </button>
    </div>
  )
