  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', gap: 12, padding: 12 }}>
        ...
        {/* Right content — padding 6px: box-shadow(5px spread) 하이라이트가 잘리지 않도록 */}
        <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12, padding: 6 }}>

          {/* Summary Bar = Instance Card */}
...
// feature ID → 탭 이름 역방향 맵 (가이드 패널 이전/다음 → 탭 자동 전환)
...
function InstanceDrawer({ instance, onClose }: { instance: DbInstance; onClose: () => void }) {
  ...
  // 드로어 마운트 시 가이드 store에 알림 (초기 탭 feature 동기화), 언마운트 시 해제
  useEffect(() => {
    openDrawer(TAB_FEATURE_MAP[tab] ?? undefined)
    return () => closeDrawer()
  // tab을 의존성에 포함하지 않음 — 마운트 시 초기값만 전달, 이후는 handleTabChange가 담당
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [openDrawer, closeDrawer])

  // 가이드 패널 이전/다음 또는 연관 기능 클릭 → 드로어 탭 자동 전환
  useEffect(() => {
    if (!guideOpen || !activeFeature) return
    const targetTab = FEATURE_TAB_MAP[activeFeature]
    if (targetTab && targetTab !== tab) setTab(targetTab)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeFeature, guideOpen])

  // 탭 클릭 시 해당 기능 가이드로 자동 포커스
  ...
  return (
    <>
      ...
      <div style={{
        ...
        transition: 'right 0.22s cubic-bezier(.4,0,.2,1), width 0.2s ease',
      }}>
        ...
        {/* Tab Content — GuideHighlight로 각 탭 콘텐츠 영역 하이라이트 연결 */}
